...
| 사용 기술 | 설명 |
| ------------ | -------------------------------- |
| 디자인 패턴| ScriptableObject를 이용하여 캐릭터 및 몬스터 데이터 관리 <br> 싱글톤을 활용하여 Manager 관리 <br> |
| 유한상태머신| Sub-State Machine을 활용한 콤보공격 구현 |
| Object Pooling| 자주 사용하는 객체는 Pool로 관리(몬스터,투사체,데미지 텍스트 등) |
| Layer| Layer를 활용하여 플레이어가 범위 내 몬스터를 인식함 <br> 몬스터는 범위 내 플레이어를 인식하면 추적함 |
